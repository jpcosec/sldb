...
This file defines the operating rituals for active work in SLDB.

## Core Rules

- No task is complete without testing.
- `desk/tasks/` contains only active work.
- Resolved tasks are deleted; history belongs in git.
...
6. BIND - Link each non-trivial implementation step to an active task.
...
3. Ensure the board reflects the active task set.
...
5. CLEAN - Delete stale desk artifacts that are no longer true.
6. DELETE - Remove the resolved task file.
7. BOARD - Remove the task from `desk/tasks/Board.md`.
8. COMMIT - Make an atomic commit if and only if the execution ritual is complete.
