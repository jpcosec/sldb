...
from nldb import AST_Handler, DataExtractor, NLDBRenderer, TemplateExtractor
...
def render_markdown(template: str, data: Dict[str, Any]) -> str:
    return NLDBRenderer().render(_RenderModel(template, data))
...
def render_model_markdown(
    ...
    return NLDBRenderer().render(model)
