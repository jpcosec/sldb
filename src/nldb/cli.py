...
def _skills_markdown() -> str:
    return files("nldb.templates").joinpath("nldb.md").read_text(encoding="utf-8")
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="nldb",
        description=(
            "nlDB treats Markdown documents as structured, typed, reversible data. "
            ...
            "markers inside the template row, and render/py/Jinja2 are non-reversible render-only paths. The stable "
            "Markdown stays literal; only the "
            "changing spans become markers. "
            "nldb then parses Markdown structurally, extracts values into model fields, renders the model back "
    ...
    init_parser = subparsers.add_parser(
        "init",
        help="Write a minimal NLDB skill file into .skills/nldb/SKILL.md.",
    )
    ...
    init_parser.add_argument(
        "--force",
        action="store_true",
        help="Overwrite an existing .skills/nldb/SKILL.md file.",
    )


    example_parser = subparsers.add_parser(
        "example",
        help="Create a comprehensive nldb example in an nldb_example folder.",
    )
...
def main(argv: Any = None) -> int:
    ...
    if args.command == "init":
        ...
        skills_dir = target_dir / ".skills" / "nldb"
    ...
    if args.command == "example":
        target_root = Path(args.path) / "nldb_example"
        target_root.mkdir(parents=True, exist_ok=True)
        
        bundle_path = files("nldb.templates").joinpath("example_bundle")
        ...
        sys.stdout.write(f"  nldb validate guide_model:NLDBGuide --input guide.input.md --pythonpath .\n")
