...
nlDB lets one Markdown document act as both a human-friendly source of truth and a machine-readable contract.
...
A StructuredNLDoc model owns a Markdown contract in __template__. Stable prose stays literal, variable spans become markers, nlDB parses the Markdown structurally, fills model fields, and can render the model back to Markdown.
...
| --- | --- | --- |
| nldb extract | Parse Markdown into model-shaped data. | nldb extract myapp.docs:RecipeDoc recipe.md recipe.json |
| nldb render | Render Markdown from JSON or YAML input. | nldb render myapp.docs:RecipeDoc recipe.yaml recipe.md |
| nldb validate | Check document and model roundtrip idempotency. | nldb validate myapp.docs:RecipeDoc --input recipe.md |
