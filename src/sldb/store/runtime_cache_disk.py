...
process extracted, keyed by document path, mtime, size and model, so the next process
...
def disk_key(rel_path: str, sig: tuple, m_name: str) -> str:
    return f"{rel_path}|{sig[1]}|{sig[2]}|{m_name}"
...
def from_disk(s_path: Path, s_name: str, m_name: str, entry: Any, sig: tuple, model_type: type) -> Any:
    ...
    hit = entries(s_path).get(disk_key(entry.path, sig, m_name))
    if hit is None:
        return None
...
def flush(s_path: Path, docs: dict[tuple, Any]) -> None:
    ...
    fresh = {disk_key(d.path, k, d.model_name): {"payload": d.payload} for k, d in docs.items() if str(d.store_path) == str(s_path)}
    _write(s_path, fresh)
