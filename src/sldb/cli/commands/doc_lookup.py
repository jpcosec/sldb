...
def find_doc(sp: Path, root: Path, idx: Any, doc_ref: str) -> tuple[Any, Any, Any]:
    ...
    by path composes each model's full documents list (a doc found by name never pays for
    that)."""
    ...
    for m_entry in idx.models:
        if doc := next((d for d in compose_documents_entries(sp, m_entry.name) if d.path == doc_ref), None):
            return m_entry, load_models_index(root / m_entry.models_index), doc
