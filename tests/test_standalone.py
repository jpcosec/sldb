...
from nldb.cli import main as cli_main
from nldb.templates.example_bundle.guide_model import NLDBGuide
from nldb.validation import (
    extract_model_data,
...
def test_cli_init_writes_skill_file(tmp_path, capsys):
    ...
    captured = capsys.readouterr()
    skill_path = tmp_path / ".skills" / "nldb" / "SKILL.md"
    ...
    assert (
        "Use `nldb` for StructuredNLDoc models and their Markdown documents." in content
    )
    assert (
    ...
    assert (
        "`nldb extract <model-ref> <input-markdown> <output-json-or-yaml>`" in content
    )
...
def test_example_bundle_roundtrips():
    bundle = files("nldb.templates.example_bundle")
    ...
    extracted = extract_model_data(NLDBGuide, input_markdown)
    rendered = render_model_markdown(NLDBGuide, input_data)
    input_valid, _ = validate_model_input_roundtrip(NLDBGuide, input_markdown)
    data_valid, _ = validate_model_data_roundtrip(NLDBGuide, input_data)

    assert extracted["title"] == "nlDB Example Guide"
    ...
    assert first_command["commands"] == "nldb extract"
    assert "# nlDB Example Guide" in rendered
...
def test_example_bundle_self_idempotency():
    ...
    first_payload = extract_model_data(NLDBGuide, input_markdown)
    rendered_markdown = render_model_markdown(NLDBGuide, first_payload)
    second_payload = extract_model_data(NLDBGuide, rendered_markdown)
