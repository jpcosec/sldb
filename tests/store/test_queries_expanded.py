...
from sldb.store.query import (
    list_structural, get_structural, glob_structural, find_structural,
    list_semantic, get_semantic, glob_semantic, find_semantic,
    list_global_semantic, get_global_semantic
)
...
def test_find_structural_invalid_address(populated_store):
    res = find_structural(populated_store, "invalid", "title='x'", mock_resolve_model_ref)
    assert res == []
...
def test_model_inheritance_query(tmp_path):
    ...
    doc1 = RuntimeDocument(
        store_name="local", store_path=store_path,
        model_name="BaseDoc", model_type=BaseDoc,
        name="doc1", path="doc1.md",
        payload={"title": "Base"},
        semantic_tags=["type.base"]
    )
    doc2 = RuntimeDocument(
        store_name="local", store_path=store_path,
        model_name="ChildDoc", model_type=ChildDoc,
        name="doc2", path="doc2.md",
    ...
    from sldb.store.query import _model_scope_docs
